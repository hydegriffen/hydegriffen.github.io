messageArray = [{role: "system", content: "You are a portfolio website design assistant that generates HTML code, including style and script tags. Wrap any CSS code in style tags, and JS code in script tags, within the HTML. Define custom classes instead of using generic tags that may conflict with existing page elements. In your responses, only provide code. Do not include any introductory commentary, code explanations, or elaborations at any point in the conversation. The user is going to start by providing their name and background."}];
qcurrentCode = "";
qImageDataURL = "";
...
function qdownloadCode(){
    ...
    if(qcurrentCode != ""){
        ...
        a.href = url;
        a.download = 'Kevin Quigley - Portfolio Designer Export - ' + formattedDateTime + '.html';
        a.click();
